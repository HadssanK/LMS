...
const CourseDetails = () => {
  ...
  try {
    ...
    const { data } = await axios.post(
       "http://localhost:5000/api/user/purches",
  ...
  // console.log(userData.enrolledCourses.includes(courseData._id));
   useEffect(() => {
    ...
    <>
      <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left">
        ...
        <div className="max-w-xl z-10 text-gray-500">
          <h1 className="md:text-course-deatails-heading-large text-details-heading-small font-semibold text-gray-800">
                        ...
                        <ul className="list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300">
                          {chapter.chapterContent.map((lecture , i ) =>(
                            <li key={i} className="flex items-start py-1 gap-2">
                              <img src={assets.play_icon} alt="play-ion" className="w-4 h-4 mt-1" />
      ...
      playerData ? <YouTube videoId={playerData.videoId} opts={{playerVars :{autoplay:1}}} iframeClassName="w-full aspect-video"/>
          ...
          <div className="p-5">
            <div className="flex item-cener gap-2">
              <img src={assets.time_left_clock_icon} alt="time" className="w-3.5"/>
            <p className="text-red-500"> <span className="font-medium">5 days </span> left at this price</p>
            </div>

             <div className="flex gap-3 items-center pt-2">
              <p className="tex-gray-800 md:text-4xl text-2xl font-semibold">{currency}{(courseData.coursePrice - courseData.discount * courseData.coursePrice / 100).toFixed(2)}</p>
              <p className="md:text-lg text-gray-500 line-through">{currency}{courseData.coursePrice}</p>
              <p className="md:text-lg text-gray-500">{courseData.discount}% off</p>

             </div>
             <div className="flex items-center tex-sm md:text-default gap-4 pt-2 md:pt-4 text-gray-500">
